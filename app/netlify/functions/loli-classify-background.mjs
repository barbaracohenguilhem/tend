...
import { classify, describeError, PROCESS } from '../../api/loli/brain.mjs';
...
export default async function (request) {
  ...
  const entry = { id, subject: email.subject, startedAt: new Date(started).toISOString(), proc: { id: PROCESS.id, calls: PROCESS.calls, uptime: Math.round(process.uptime()) } };
  try {
  ...
  } catch (e) { entry.error = describeError(e); }
