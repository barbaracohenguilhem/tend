...
export default async function (request) {
  ...
  if (mode === 'messages') {
    ...
    const base = new URL(route().baseURL); log.route = route();
    while (Date.now() < end) {
      const sample = { uptime: Math.round(process.uptime()), at: new Date().toISOString(), ...(await messages(base, model)) };
...
function messages(base, model) {
  const host = base.hostname, path = `${base.pathname.replace(/\/$/, '')}/v1/messages`;
  return new Promise(resolve => {
    const started = Date.now();
    ...
    const req = https.request({ host, port: 443, path, method: 'POST', agent, timeout: 30000,
